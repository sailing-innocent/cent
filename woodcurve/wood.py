...
image = ti.field(dtype=ti.f32, shape=[GRID_SIZE,GRID_SIZE])
...
def init():
    ...
    for i in position:
        ...
        position[i] = ti.Vector([GRID_SIZE/2 + 2 * (ti.random()-0.5) * INIT_SIZE, GRID_SIZE/2 + 2 * (ti.random()-0.5) * INIT_SIZE]) 
...
@ti.kernel
def step(phase: ti.i32, temp:ti.f32):
    # move
    for i in position:
        ...
        if l < c < r:
            ...
            ang = -ang
        pos += ti.Vector([ti.cos(ang), ti.sin(ang)]) * MOVE_STEP * (sigmoid(temp)+0.1)
    ...
    for i, j in ti.ndrange(GRID_SIZE, GRID_SIZE):
        ...
        a *= EVAPORATION / 9.0
        if (image[i,j] < MAX_COLOR and a > MIN_COLOR):
            image[i,j] += a/10
        grid[1 - phase, i, j] = a

    # color image using grid
